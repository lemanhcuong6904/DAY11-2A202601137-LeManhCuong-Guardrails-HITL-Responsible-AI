...
async def generate_ai_attacks() -> list:
    ...
    try:
        ...
        generate_ai_attacks.last_error = (
            "The OpenAI SDK is not installed. Run `pip install -r requirements.txt`."
        )
        print(f"AI attack generation unavailable: {generate_ai_attacks.last_error}")
        return []
    ...
    try:
        ...
        generate_ai_attacks.last_error = f"{type(exc).__name__}: {exc}"
        print(f"AI attack generation failed: {generate_ai_attacks.last_error}")
        return []
    ...
    try:
        ...
        ai_attacks = parse_ai_attacks(text)
        generate_ai_attacks.last_error = None
        ...
        generate_ai_attacks.last_error = f"{type(e).__name__}: {e}"
        print(f"Error parsing: {generate_ai_attacks.last_error}")
...
def parse_ai_attacks(raw_text: str) -> list[dict]:
    """Parse and validate AI-generated attacks from imperfect model output."""
    payload = _parse_json_payload(raw_text)
    if isinstance(payload, dict):
        for key in ("attacks", "prompts", "items", "data"):
    ...
    if not isinstance(payload, list):
        raise ValueError("AI response JSON must be an array or object containing an array")
    ...
    for idx, item in enumerate(payload, 1):
        ...
        if not prompt:
            continue
        technique = str(
            item.get("technique") or item.get("type") or item.get("category") or "ai_generated"
        ).strip()
        parsed.append(
            {
                "id": item.get("id") or f"ai_{idx}",
                "type": technique,
                "technique": technique,
                "prompt": prompt,
                "input": prompt,
                "category": technique,
                "target": item.get("target") or "secret_extraction",
                "why_it_works": item.get("why_it_works") or "Generated by AI red-team prompt.",
            }
        )
    return parsed
...
def save_attack_results(
    *,
    ...
    payload = {
        ...
        "generation_error": generation_error or getattr(generate_ai_attacks, "last_error", None),
        "summary": {
